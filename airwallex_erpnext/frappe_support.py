...
try:
    import frappe
except ImportError:  # pragma: no cover - pure unit tests
...
def get_client(settings):
    ...
    return AirwallexClient(
        ...
        api_key=api_key,
        login_as=settings.account_id or None,
        timeout_seconds=int(settings.timeout_seconds or 30),
    )
...
def log_sync(settings_name: str, module: str, status: str, **values: Any):
    ...
    doc = f.get_doc(
        {
            "doctype": "Airwallex Sync Log",
            "settings": settings_name,
            "module": module,
            "status": status,
            "started_at": f.utils.now_datetime(),
            **values,
        }
    )
